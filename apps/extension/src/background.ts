  ...
  try {
    switch (request.method) {
      ...
      case "bookmarks.search":
        return { id: request.id, type: "response", ok: true, result: await search(request.params.query) };
      case "bookmarks.get":
        return { id: request.id, type: "response", ok: true, result: await get(request.params.id) };
      case "bookmarks.children":
        return { id: request.id, type: "response", ok: true, result: await getChildren(request.params.id) };
    }
  } catch (error) {
